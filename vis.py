...
chords_config = {"color": "Spectral", # Spectral is essentially the color scale (from https://github.com/d3/d3-scale-chromatic) that we are configuring the chords to use. (This is what the 'value' parameter represents)
                 "opacity": 0.8,
                 'tooltipContent': {
                     'source': 'source',
                     'sourceID': 'id',
        ...
        if input_dict["Chords"][i]["value"] in value_list:
            ...
            for j in range(i, len(input_dict["Chords"])):
                ...
                if input_dict["Chords"][j]["source"]["id"] == current_target_id and input_dict["Chords"][j]["target"]["id"]==current_source_id:
                    ...
                    input_dict["Chords"][i]["source"]["end"]=float(input_dict["Chords"][i]["source"]["start"])+0.000000006 #this represents a formerly redundant chord
...
if __name__ == '__main__':
    ...
    """
        ...
        file_name_limit (required): limit to access named files properly (run 39 has file name limit 2; run 104 has file name limit 3; etc.)
